...
def scaled_dot_product_two_loop_single(query: Tensor, key: Tensor, value: Tensor) -> Tensor:
    ...
    K, M = query.shape
    similarities = torch.zeros(size = (K, K), dtype = query.dtype) 
    out = torch.zeros(size = (K, M), dtype = query.dtype)

    for i in range(K): 
        q = query[i,:] 
        for j in range(K): 
            k = key[j,:] 
            dot_prod = torch.sum(q * k) 
            similarities[i, j] = dot_prod / torch.sqrt(torch.tensor(M))

    attention_weights = torch.softmax(similarities, dim = 1) 
     
    for i in range(K): 
        for j in range(K): 
            out[i,:] += value[j,:] * attention_weights[i,j] 
...
def scaled_dot_product_two_loop_batch(query: Tensor, key: Tensor, value: Tensor) -> Tensor:
    ...
    for n in range(N): 
        ...
        k = key[n]
        v = value[n]
        out[n] = scaled_dot_product_two_loop_single(q, k, v) # K M
...
class SelfAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Tensor = None) -> Tensor:
        ...
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(self.q, self.k, self.v)
